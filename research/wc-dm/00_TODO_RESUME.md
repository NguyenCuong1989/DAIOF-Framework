...
> **FLOW ENTRYPOINT — read this file first.**
...
These values are state, not targets.
...
`K_z(R,z)` → source → provenance → validated relation → coupling to `x` → admissibility.

**Forbidden before coupling PASS:** computation, new constraint, intersection, target fitting.

## 4. GAP protocol

`GAP → REQUIRED_OBJECT → SOURCE → PROVENANCE → COUPLING → ADMISSION → COMPUTATION → FALSIFICATION → DEPENDENCY → INTERSECTION → VERIFY → SAVE → COMMIT → RESUME`

If the required object does not exist in admissible sources, record a DATA/COUPLING/MODEL GAP. Do not guess or silently substitute.
...
A task is not complete because it is described. It is complete only when the required artifact exists, its state is recorded, verification is performed, and the checkpoint is committed.

**NEXT:** execute G3 read-only audit only.